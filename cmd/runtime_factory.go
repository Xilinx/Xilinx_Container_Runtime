...
func findRuntime(runtimeCandidates []string) (string, error) {
	// Get System path or default "/usr/bin:/usr/sbin"
	path := os.Getenv("PATH")
	logger.Infof("System path: %s", path)
	...
	for _, candidate := range runtimeCandidates {
		...
		for _, dir := range filepath.SplitList(path) {
			...
			runcPath := filepath.Join(dir, candidate)
			d, err := os.Stat(runcPath)
			if err != nil {
				continue
			}
		...
		logger.Warnf("Runtime binary '%v' not found", candidate)
	}
